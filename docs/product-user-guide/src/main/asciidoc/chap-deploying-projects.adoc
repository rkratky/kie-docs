...
. In Business Central, select *Menu* -> *Design* -> *Projects*.
...
The following sections describe the features provided by the options available under *Menu* → *Manage* in Business Central.
...
Additionally, Business Central enables you to easily sort and filter a list of tasks for any given process. You can create custom filters that enable you to define queries by user, business attributes (such as amount, customer segmentation), Process ID, correlation key, and so on.
...
User Tasks are displayed in the Tasks List perspective. To display the Tasks Lists perspective, select *Menu* -> *Track* -> *Task Lists*.
...
. In Business Central, select *Menu* -> *Track* -> *Task Lists*
